<template>
  <v-layout column>
    <div v-if="s.loading_getMosaicsByAddress">
    ...
    && s.assets.length > 0">
      <div>
        <v-tabs v-model="active" color="cyan" dark slider-color="yellow">
          ...
          <v-tab-item :key="1">
            <AssetTab :assets="filterZeros(s.assets)"/>
          </v-tab-item>
          <v-tab :key="2" ripple>Owned Assets</v-tab>
          <v-tab-item :key="2">
            <AssetTab :assets="filterByOwner(s.assets, s.activeWallet.account.address.pretty())"/>
          </v-tab-item>
        </v-tabs>
      </div>
    </div>
